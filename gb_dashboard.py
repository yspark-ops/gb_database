...
# --- 3. 데이터 전처리 (2026년 정밀 타격 로직) ⭐ ---
def preprocess_final(df, qty_col, rev_col, month_col):
    if df.empty: return df
    
    # 1) 연도 추출 로직 (매출인식_기준일(출고일) 우선 적용)
    target_date_col = "매출인식_기준일(출고일)"
    
    if target_date_col in df.columns:
        # "2026. 4. 1" 형식에서 맨 앞의 4자리 숫자만 추출
        df['year_val'] = df[target_date_col].astype(str).str.extract(r'^(\d{4})').fillna(0).astype(int)
    ...
    # 2) 숫자형 변환 (콤마, 원화기호 등 제거)
    for col in [qty_col, rev_col]:
        if col in df.columns:
            # 숫자가 아닌 문자(콤마 등)를 모두 제거하고 숫자로 변환
...
df_raw_all = preprocess_final(df_raw_all, "제품판매수량", "매출취합용_공급가액(원화기준)", "월")
df_master_all = preprocess_final(df_master_all, "출고_수량", "매출액", "월")
...
with st.sidebar:
    ...
    # 데이터에 존재하는 모든 연도 수집
    raw_y = set(df_raw_all['year_val'].unique())
    mst_y = set(df_master_all['year_val'].unique())
    all_years = sorted([y for y in list(raw_y | mst_y) if y > 2000], reverse=True)
...
# 💡 진단 도구 (데이터가 0일 때만 자동 오픈)
if f_raw.empty and selected_year == 2026:
    with st.expander("🔍 2026 데이터 인식 진단 중...", expanded=True):
        st.write("실제 DB에서 찾은 연도들:", sorted(df_raw_all['year_val'].unique()))
        if not df_raw_all.empty:
            st.write("데이터 샘플 (날짜 컬럼값):", df_raw_all[["월", "매출인식_기준일(출고일)"]].head(2))
        st.info("💡 위 목록에 2026이 없다면 DB에 데이터가 없거나, 컬럼값이 '2026'으로 시작하지 않는 것입니다.")
...
if not f_raw.empty:
    ...
    with c3:
        st.markdown(f"### ■ {selected_year} Sell-Out")
    ...
    st.info("선택한 연도/거래처에 해당하는 데이터가 없습니다.")