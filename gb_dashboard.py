...
st.caption(f"기준 컬럼: 매출인식_기준일(출고일) | 현재 {selected_year}년 데이터 조회 중")
...
with k4: 
    ...
    month_str = ", ".join([f"{m}월" for m in months]) if months else "집계전"
    st.markdown(f'<div class="metric-card"><div class="metric-label">데이터 확인 월</div><div class="metric-value">{month_str}</div></div>', unsafe_allow_html=True)
...
if not f_raw.empty:
    ...
    st.info(f"{selected_year}년에 해당하는 데이터가 '매출인식_기준일(출고일)' 컬럼에 없습니다.")